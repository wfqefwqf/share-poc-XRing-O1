...
| 真正知识空白 | **waiter_thread 的 task_struct 地址未知**，PI 链 target 无法设置 |
...
## 3. 重要认知纠正：spray fake cred 方案

### 3.1 文档描述与反汇编证据的矛盾

`help_post.txt:18` 与 `README.md:124` 都说：

> "cred 直写路线是直接写 task->real_cred 和 task->cred 等于 init_cred"
...
**所以正确方案是让 cred 指向 spray 页里伪造的 cred，不是内核的 init_cred**。文档里"等于 init_cred"是简化描述（伪造 cred 的内容复制自 init_cred）。
...
**waiter_thread 的 task_struct 地址如何获取**是项目最大的待解谜题。

可能的猜测（**未验证**）：

1. 可能存在某个我们没注意的侧信道
2. 可能在 futex_wait_requeue_pi 触发后，通过某种方式让 task_struct 落在可预测地址（KASLR=0 + heap spray？）
3. 可能根本不需要 task_struct 地址 —— 走了完全不同的路径（不写 cred，改写别的内核对象）
4. 可能依赖一个我们漏看的设备接口
...
### 6.3 ★最高杠杆: 寻找 task_struct 泄露方法

P3 wall（task_struct leak）是 P1 的真正 blocker。**所有候选人（kernelsnitch、binder、fsnotify、procfs、uhid fops）都已被排除**。下一步要么：

- (A) 反汇编 rt_mutex_adjust_prio_chain 全貌，寻找 PI 链传播中的 task_struct 泄露路径
- (B) 设计全新的侧信道扫 task_struct（工程量大）

**推荐 A 路径**：PI 链传播过程中 task_struct 指针很可能出现在某处，比从零设计侧信道成本低很多。
