...
PI 链遍历到伪造节点时，`rb_insert` 写入的是 **节点地址 (node addr)**，不是 `write_pc`：

反汇编 `rt_mutex_adjust_prio_chain` (见 `chain_disasm.txt`)：
- line 193     `str x25, [x11, x13]`          → `[parent.rb_child] = node`
- line 407-409 `str x0, [x11, x13]`, `x0 = node.pi_tree_entry`
  → `[parent.rb_child] = &node.pi_tree_entry`

即：`*(target) = node_addr`，其中：
- `node_addr` = 被插入节点 (spray 对象) 的 `pi_tree_entry` 地址（**固定已知，由 spray 决定**）
- `target` = 任意地址，由 `fake_w0.pi_tree_entry.rb_left / rb_right`（= `target-8` / `target-0x10`）决定（parent 是我们能控的节点）
- `write_pc`（= `fake_w0.pi_tree_entry.__rb_parent_color`）只是节点的父色字段，**不会被写到 target**

**关键推论**：既然写的是节点地址，要让 `task->cred` 指向有效 cred，spray 页面必须在该节点偏移处包含一份**伪造 cred**（`uid=0, gid=0, cap=全开, securebits=0, ...`）。当前 `prepare_skb_payload` 把 `fake_w0.pi_tree_entry` 填成 `{write_pc, write_right, write_left}`，**不含 cred**——这是下一步必须补齐的部分（在 spray 页安排伪造 cred，并让插入节点落在它上面）。
...
kernelsnitch leak mm_struct
    ↓
PI 链写 ashmem_misc_fops = <节点地址> (指向含 fake_fops 的 spray 页)
    ↓
...
**问题**：fops 路线绕了一大圈，且依赖 `configfs_read_once` 做验证读。另注（见 §3.1）：PI 链写入的是**节点地址**，要让 `ashmem_misc_fops` 真正指向可用 fops 表，spray 页须在节点偏移处放一份完整 `file_operations`——当前布局未做到，故即便 fops 写入成功也未必能 bootstrap 任意读写。
...
**写入机制已验证**：`set_pselect_write(target-8, value, 0)` 经 `prepare_skb_payload` 构造 `fake_w0.pi_tree.rb_left = target-8`，PI 链触发后 `*(target) = node_addr`（**节点地址**，非 `value`；见 §3.1）。
